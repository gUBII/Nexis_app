...
const Quote = ({route, navigation }) => {
  ...
  const handleQuoteEdit = (quotesItem) => {
    navigation.navigate('EditQuote', { quotes: quotesItem });
  };
  ...
  const handleQuotesSubmit = async () => {
    ...
    formDataToSubmit.append('log_type', 'QUOTE');
    ...
    try {
      const response = await axios.post('https://app.nexis365.com/api/quote', formDataToSubmit, {
        headers: {
      ...
      if (response.data.success) {
        Alert.alert('Success', 'Quote log saved successfully!');
        ...
        Alert.alert('Error', response.data.message || 'Failed to save quotes log.');
      }
    } catch (error) {
      console.error(error);
   ...
   useFocusEffect(
      useCallback(() => {
        const fetchQuotes = async () => {
          ...
          try {
            const response = await axios.get("https://app.nexis365.com/api/get-quotes", {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.tabContainer}>
         <TouchableOpacity
           ...
           >
            <Text style={[styles.tabButtonText, tab === 'create' && styles.activeTabButtonText]}>Create Quote</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            onPress={() => setTab('view')}
          >
            <Text style={[styles.tabButtonText, tab === 'view' && styles.activeTabButtonText]}>View Quote</Text>
           </TouchableOpacity>
        </View>
        ...
        {tab === 'create' ? (
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleQuotesSubmit}>
              ...
              <View
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: 'bold', color: theme === 'dark' ? '#fff' : '#000' }}>
                    Log ID: {quotesItem.logid}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Date: {new Date(quotesItem.date * 1000).toLocaleDateString()}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Title: {quotesItem.title}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Detail: {quotesItem.detail}
                  </Text>
                </View>
                    
                {/* Edit button */}
                <TouchableOpacity
                  onPress={() => handleQuoteEdit(quotesItem)}
                  style={{
                    padding: 8,  
