...
const Minutes = ({route, navigation }) => {
  ...
  const handleMinutesEdit = (minutesItem) => {
    navigation.navigate('EditMinutes', { minutes: minutesItem });
  };
  ...
  const handleMinuteSubmit = async () => {
    ...
    formDataToSubmit.append('log_type', 'MINUTES');
    ...
    try {
      const response = await axios.post('https://app.nexis365.com/api/minutes', formDataToSubmit, {
        headers: {
      ...
      if (response.data.success) {
        Alert.alert('Success', 'Minutes log saved successfully!');
        ...
        Alert.alert('Error', response.data.message || 'Failed to save minutes log.');
      }
    } catch (error) {
      console.error(error);
   ...
   useFocusEffect(
      useCallback(() => {
        const fetchMinutes = async () => {
          ...
          try {
            const response = await axios.get("https://app.nexis365.com/api/get-minutes", {
              params: { ref_db, userid }
            });
    
            if (response.data.success) {
              setMinutes(response.data.data);
            }
          } catch (error) {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.tabContainer}>
         <TouchableOpacity
           ...
           >
            <Text style={[styles.tabButtonText, tab === 'create' && styles.activeTabButtonText]}>Create Minutes</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            onPress={() => setTab('view')}
          >
            <Text style={[styles.tabButtonText, tab === 'view' && styles.activeTabButtonText]}>View Minutes</Text>
           </TouchableOpacity>
        </View>
        ...
        {tab === 'create' ? (
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleMinuteSubmit}>
              ...
              <View
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: 'bold', color: theme === 'dark' ? '#fff' : '#000' }}>
                    Log ID: {minutesItem.logid}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Date: {new Date(minutesItem.date * 1000).toLocaleDateString()}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Title: {minutesItem.title}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Detail: {minutesItem.detail}
                  </Text>
                </View>
                    
                {/* Edit button */}
                <TouchableOpacity
                  onPress={() => handleMinutesEdit(minutesItem)}
                  style={{
                    padding: 8,  
