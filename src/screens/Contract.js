...
const Contract = ({route, navigation }) => {
  ...
  const handleContractEdit = (contractsItem) => {
    navigation.navigate('EditContract', { contracts: contractsItem });
  };
  ...
  const handleContractsSubmit = async () => {
    ...
    formDataToSubmit.append('log_type', 'CONTRACT');
    ...
    try {
      const response = await axios.post('https://app.nexis365.com/api/contract', formDataToSubmit, {
        headers: {
      ...
      if (response.data.success) {
        Alert.alert('Success', 'Contract log saved successfully!');
        ...
        Alert.alert('Error', response.data.message || 'Failed to save contracts log.');
      }
    } catch (error) {
      console.error(error);
   ...
   useFocusEffect(
      useCallback(() => {
        const fetchContracts = async () => {
          ...
          try {
            const response = await axios.get("https://app.nexis365.com/api/get-contracts", {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.tabContainer}>
         <TouchableOpacity
           ...
           >
            <Text style={[styles.tabButtonText, tab === 'create' && styles.activeTabButtonText]}>Create Contract</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            onPress={() => setTab('view')}
          >
            <Text style={[styles.tabButtonText, tab === 'view' && styles.activeTabButtonText]}>View Contract</Text>
           </TouchableOpacity>
        </View>
        ...
        {tab === 'create' ? (
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleContractsSubmit}>
            <Text style={styles.submitButtonText}>Submit</Text> 
             ...
             <View style={{ marginTop: 20 }}>
              ...
              contracts.map((contractsItem, index) => (
              <View
                key={index}
                style={{
                flexDirection: 'row',
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: 'bold', color: theme === 'dark' ? '#fff' : '#000' }}>
                    Log ID: {contractsItem.logid}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Date: {new Date(contractsItem.date * 1000).toLocaleDateString()}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Title: {contractsItem.title}
                  </Text>
                  <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                    Detail: {contractsItem.detail}
                  </Text>
                </View>
                    
                {/* Edit button */}
                <TouchableOpacity
                  onPress={() => handleContractEdit(contractsItem)}
                  style={{
                    padding: 8,  
