...
const EditDeal = ({ route, navigation }) => {
  ...
  const [title, setTitle] = useState(deals.title || '');
  const [detail, setDetail] = useState(deals.detail || '');
  const [documentFile, setDocumentFile] = useState(null);
  const [date, setDate] = useState(new Date(deals.date * 1000));
  ...
  const handleUpdate = async () => { 
    ...
    formData.append('id', String(deals.id));
    ...
    try {
      const response = await axios.post(
        'https://app.nexis365.com/api/update-deal',
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        Alert.alert("Updated", "Deal updated successfully");
        navigation.goBack();
      } else {
        Alert.alert("Error", response.data.message || "Failed to update Deal");
      }
    } catch (error) {
      console.error(error);
