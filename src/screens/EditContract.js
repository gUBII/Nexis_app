...
const EditContract = ({ route, navigation }) => {
  const { theme } = useTheme();
  const { contracts } = route.params;
  const [title, setTitle] = useState(contracts.title || '');
  const [detail, setDetail] = useState(contracts.detail || '');
  const [documentFile, setDocumentFile] = useState(null);
  const [date, setDate] = useState(new Date(contracts.date * 1000));
  ...
  const handleUpdate = async () => { 
    ...
    formData.append('id', String(contracts.id));
    ...
    try {
      const response = await axios.post(
        'https://app.nexis365.com/api/update-contract',
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        Alert.alert("Updated", "Contract updated successfully");
        navigation.goBack();
      } else {
        Alert.alert("Error", response.data.message || "Failed to update Contract");
      }
    } catch (error) {
      console.error(error);
      ...
      {showDatePicker && (
        ...
        />
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleUpdate}>
            <Text style={styles.submitButtonText}>Update Contract</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
      <BottomTabBar />
