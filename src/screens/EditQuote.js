...
const EditQuote = ({ route, navigation }) => {
  ...
  const [title, setTitle] = useState(quotes.title || '');
  const [detail, setDetail] = useState(quotes.detail || '');
  const [documentFile, setDocumentFile] = useState(null);
  const [date, setDate] = useState(new Date(quotes.date * 1000));
  ...
  const handleUpdate = async () => { 
    ...
    formData.append('id', String(quotes.id));
    ...
    try {
      const response = await axios.post(
        'https://app.nexis365.com/api/update-quote',
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        Alert.alert("Updated", "Quote updated successfully");
        navigation.goBack();
      } else {
        Alert.alert("Error", response.data.message || "Failed to update Quote");
      }
    } catch (error) {
      console.error(error);
      ...
      {showDatePicker && (
        ...
        />
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleUpdate}>
            <Text style={styles.submitButtonText}>Update Quote</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
      <BottomTabBar />
