...
const Deal = ({route, navigation }) => { 
  ...
  const handleDealEdit = (dealsItem) => {
    navigation.navigate('EditDeal', { deals: dealsItem });
  };
  ...
  const handleDealsSubmit = async () => {
    ...
    formDataToSubmit.append('log_type', 'DEAL');
    ...
    try {
      const response = await axios.post('https://app.nexis365.com/api/deal', formDataToSubmit, {
        headers: {
      ...
      if (response.data.success) {
        Alert.alert('Success', 'Deal log saved successfully!');
        ...
        Alert.alert('Error', response.data.message || 'Failed to save deals log.');
      }
    } catch (error) {
      console.error(error);
   ...
   useFocusEffect(
      useCallback(() => {
        const fetchDeals = async () => {
          ...
          try {
            const response = await axios.get("https://app.nexis365.com/api/get-deals", {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.tabContainer}>
         <TouchableOpacity
           ...
           >
            <Text style={[styles.tabButtonText, tab === 'create' && styles.activeTabButtonText]}>Create Deal</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            onPress={() => setTab('view')}
          >
            <Text style={[styles.tabButtonText, tab === 'view' && styles.activeTabButtonText]}>View Deal</Text>
           </TouchableOpacity>
        </View>
        ...
        {tab === 'create' ? (
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleDealsSubmit}>
                ...
                <View
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontWeight: 'bold', color: theme === 'dark' ? '#fff' : '#000' }}>
                      Log ID: {dealsItem.logid}
                    </Text>
                    <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                      Date: {new Date(dealsItem.date * 1000).toLocaleDateString()}
                    </Text>
                    <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                      Title: {dealsItem.title}
                    </Text>
                    <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                      Detail: {dealsItem.detail}
                    </Text>
                  </View>
                      
                  {/* Edit button */}
                  <TouchableOpacity
                    onPress={() => handleDealEdit(dealsItem)}
                    style={{
                      padding: 8,  
