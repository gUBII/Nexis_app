...
const Cases = ({route, navigation }) => {
  ...
  const handleCasesEdit = (casesItem) => {
    navigation.navigate('EditCases', { cases: casesItem });
  };
  ...
  const handleCasesSubmit = async () => {
    ...
    formDataToSubmit.append('log_type', 'CASES');
    ...
    try {
      const response = await axios.post('https://app.nexis365.com/api/cases', formDataToSubmit, {
        headers: {
      ...
      if (response.data.success) {
        Alert.alert('Success', 'Cases log saved successfully!');
        ...
        Alert.alert('Error', response.data.message || 'Failed to save cases log.');
      }
    } catch (error) {
      console.error(error);
   ...
   useFocusEffect(
      useCallback(() => {
        const fetchCases = async () => {
          ...
          try {
            const response = await axios.get("https://app.nexis365.com/api/get-cases", {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.tabContainer}>
         <TouchableOpacity
           ...
           >
            <Text style={[styles.tabButtonText, tab === 'create' && styles.activeTabButtonText]}>Create Cases</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            onPress={() => setTab('view')}
          >
            <Text style={[styles.tabButtonText, tab === 'view' && styles.activeTabButtonText]}>View Cases</Text>
           </TouchableOpacity>
        </View>
        ...
        {tab === 'create' ? (
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleCasesSubmit}>
            ...
            <View
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontWeight: 'bold', color: theme === 'dark' ? '#fff' : '#000' }}>
                  Log ID: {casesItem.logid}
                </Text>
                <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                  Date: {new Date(casesItem.date * 1000).toLocaleDateString()}
                </Text>
                <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                  Title: {casesItem.title}
                </Text>
                <Text style={{ color: theme === 'dark' ? '#fff' : '#000' }}>
                  Detail: {casesItem.detail}
                </Text>
              </View>
                  
              {/* Edit button */}
              <TouchableOpacity
                onPress={() => handleCasesEdit(casesItem)}
                style={{
                  padding: 8,  
