...
const EditCall = ({ route, navigation }) => {
  const { theme } = useTheme();
  const { calls } = route.params;
  const [title, setTitle] = useState(calls?.title || '');
  const [detail, setDetail] = useState(calls?.detail || '');
  const [documentFile, setDocumentFile] = useState(null);
  const [date, setDate] = useState(new Date(calls?.date * 1000));
  ...
  const handleUpdate = async () => { 
    ...
    formData.append('id', String(calls.id));
    ...
    try {
      const response = await axios.post(
        'https://app.nexis365.com/api/update-call',
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        Alert.alert("Updated", "Call updated successfully");
        navigation.goBack();
      } else {
        Alert.alert("Error", response.data.message || "Failed to update Call");
      }
    } catch (error) {
      console.error(error);
      ...
      {showDatePicker && (
        ...
        />
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleUpdate}>
            <Text style={styles.submitButtonText}>Update Call</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
      <BottomTabBar />
