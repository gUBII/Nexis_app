...
const EditMinutes = ({ route, navigation }) => {
  const { theme } = useTheme();
  const { minutes } = route.params;
  const [title, setTitle] = useState(minutes?.title || '');
  const [detail, setDetail] = useState(minutes?.detail || '');
  const [documentFile, setDocumentFile] = useState(null);
  const [date, setDate] = useState(new Date(minutes?.date * 1000));
  ...
  const handleUpdate = async () => { 
    ...
    formData.append('id', String(minutes.id));
    ...
    try {
      const response = await axios.post(
        'https://app.nexis365.com/api/update-minute',
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        Alert.alert("Updated", "Minutes updated successfully");
        navigation.goBack();
      } else {
        Alert.alert("Error", response.data.message || "Failed to update Minutes");
      }
    } catch (error) {
      console.error(error);
      ...
      {showDatePicker && (
        ...
        />
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleUpdate}>
            <Text style={styles.submitButtonText}>Update Minutes</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
      <BottomTabBar />
